...
export class WordParser implements Parser {
    ...
    public read(word: string): SyntaxNode {
        ...
        if (!isNaN(word.trim() as any))
            return new NumberNode(parseInt(word));
        if (typeof word.trim() === "string")
...
export class LoopParser implements Parser {
    ...
    public read(word: string): SyntaxNode {
        ...
        switch (this.state) {
            ...
            case "Body":
                let until = null, from = null, perform = null;
                if (this.parts.length < 1) throw new Error(`Loop received no information of the loop conditions, i.e. how long to loop`);

                let getFromNode = (node : SyntaxNode) => {
                    let isAssignmentNode = (node : SyntaxNode) => {
                        return node.type === "AssignmentNode";
                    }

                    let getVariable = (node : AssignmentNode) : SyntaxNode => {
...
export class OperationParser implements Parser {
    ...
    public read(word: string): SyntaxNode {
        ...
        let currentNode = this.service.nodes.pop();
        ...
        if (currentNode.type === new AssignmentNode(null, null).type) {
            ...
            let result = new AssignmentNode(assignment.variableName, new OperationNode(this.current.operator,
                assignment.value,
                this.service.clone().revert(1).parseOne()));
            if (loggingLevel === "verbose") console.log(`Operation Parser result: 
${JSON.stringify(result, undefined, 2)}`)
            this.isDone = true;
            return result;
        }
        else if (currentNode.type === new UnwrapSequenceNode(null).type) {
            let unwrap = currentNode as UnwrapSequenceNode;
            for (let i = 0; i < unwrap.children.length; i++) {
                let current = unwrap.children[i];
...
export class ComparisonParser implements Parser {
    ...
    public read(word: string): SyntaxNode {
        ...
        if (currentNode.type === ExpandVariableNode.name) {
            let result = new ComparingNode(this.current.comparator, 
                currentNode,
                this.service.clone().revert(1).parseOne());
            return result;
        }
            ...
            for (let i = 0; i < unwrap.children.length; i++) {
                ...
                if (current.type === new AssignmentNode(null, null).type) {
                    assignment = current as AssignmentNode;
                    let result = new AssignmentNode(assignment.variableName, new ComparingNode(this.current.comparator,
                        assignment.value,
                        this.service.clone().revert(1).parseOne()));
                    unwrap.children[i] = result;

                    if (loggingLevel === "verbose") console.log(`Comparator Parser result: 
${JSON.stringify(unwrap, undefined, 2)}`);
                    this.isDone = true;
                    return unwrap;
                }
...
export class ScopeParser implements Parser {
    ...
    public activate(word: string): boolean {
        let activate = this.keywords.some(_ => _ === word.trim())
        this.isDone = !activate;
        this.started = false;
        this.depth = 0;
        return activate;
    ...
    public read(word: string): SyntaxNode {
        ...
        if (Keywords.end(this.service.language).some(_ => word.endsWith(_))) {
            if (this.depth > 0) {
                ...
                let parser = new ParsingService(this.scope, this.service.language);
    ...
    public read(word: string): SyntaxNode {
        ...
        switch (this.state) {
            ...
            case "Body":
                ...
                if (loggingLevel === "verbose") console.log(`Read a full body. 
Result:
${JSON.stringify(this.current, undefined, 2)}`)
                return null;
        }

        if (loggingLevel === "verbose") console.log(`Think I'm done with switch scope. 
Result:
...
export class SwitchParser implements Parser {
    ...
    public activate(word: string): boolean {
        let activate = this.keywords.some(_ => _ === word)
        this.isDone = !activate;
        this.state = "Comparer";
        this.comparisonNodes = [];
        return activate;
    ...
    public read(word: string): SyntaxNode {
        ...
        switch (this.state) {
            ...
            case "ComparisonScope":
                let service = this.service.clone().revert(1);
                service.setParser(new SwitchScopeParser(service));
                let result = service.parseOne();
    ...
    public read(word: string): SyntaxNode {
        ...
        switch (this.state) {
            ...
            case "Body":
                ...
                let scope = this.service.clone().revert(1).parseOne();
    ...
    public read(word: string): SyntaxNode {
        ...
        switch (this.state) {
            ...
            case "AssignVariables":
                ...
                this.functionArguments.push(new AssignmentNode(this.currentArgument,
                    this.service.clone().revert(1).parseOne()));
...
export class ClassParser implements Parser {
    ...
    public activate(word: string): boolean {
        ...
        this.state = "Type";
        if (activate)
            if (loggingLevel === "verbose") console.log("ClassParser activated for " + word);
        return activate;
    }
...
export class AssignmentParser implements Parser {
    ...
    public activate(word: string): boolean {
        ...
        this.isDone = !activate;
        if (activate)
            if (loggingLevel === "verbose") console.log("AssignmentParser activated for " + word);
        return activate;
    }
    ...
    public read(word: string): SyntaxNode {
        ...
        let variableNameStatement = this.service.wordHistory[this.service.wordHistory.length - 3];
        let variableName = this.service.clone().parseWord(variableNameStatement);
        let result = this.service.clone().revert(1).parseOne();
        let unwrapAssignmentParser = new UnwrapAssignmentParser(result);
        this.isDone = true;
...
export class VariableParser implements Parser {
    ...
    public read(word: string): SyntaxNode {
        ...
        let result = this.service.clone().parseWord(word);
...
export class ExpandParser implements Parser {
    ...
    public read(word: string): SyntaxNode {
        ...
        let unwrapParser = new UnwrapExpansionParser();
